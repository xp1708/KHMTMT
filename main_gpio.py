...
# GPIO LED / BUZZER
# =========================
GPIO_LED_PIN             = 17    # Physical Pin 11 — LED vang canh bao
GPIO_BUZZER_PIN          = 27    # Physical Pin 13 — Active buzzer
ALERT_PHASE1_SECONDS     = 2.0   # 0->2s: LED nhap nhay 0.5s, Buzzer im
LED_BLINK_FAST_INTERVAL  = 0.5   # Phase 1: bật/tắt mỗi 0.5s
LED_BLINK_SLOW_INTERVAL  = 1.0   # Phase 2: bật/tắt mỗi 1.0s
BUZZER_BEEP_INTERVAL     = 1.0   # Phase 2: buzzer keu moi 1.0s
...
class HardwareAlertController:
    """
    Dieu khien LED + Buzzer tren thread rieng, khong block vong lap camera.

    Phase 1 (0 -> 2s) : LED nhay 0.5s, Buzzer im
    Phase 2 (> 2s)    : LED nhay 1s + Buzzer keu 1s
    Alert bien mat    : Tat ca ngay lap tuc
    """

    def __init__(self, led_pin=GPIO_LED_PIN, buzzer_pin=GPIO_BUZZER_PIN):
    ...
    def _init_gpio(self):
        try:
            ...
            self._gpio_ok = True
            print(f"[GPIO] LED=GPIO{self.led_pin}, Buzzer=GPIO{self.buzzer_pin} OK")
        except ImportError:
            print("[GPIO] RPi.GPIO khong tim thay — bo qua phan cung")
        except Exception as e:
            print(f"[GPIO] Loi setup: {e}")

    def set_alert(self, active):
        """Goi moi frame tu main loop. Thread-safe."""
        with self._lock:
    ...
    def stop(self):
        ...
        if self._gpio_ok:
            try:
                ...
                self._GPIO.cleanup([self.led_pin, self.buzzer_pin])
    ...
    def _run(self):
        GPIO = self._GPIO
        led, buz = self.led_pin, self.buzzer_pin
        while not self._stop_evt.is_set():
            ...
            tick    = time.time()
            if elapsed <= ALERT_PHASE1_SECONDS:
                # Phase 1: LED nhap nhay 0.5s, Buzzer im
                on = (tick % (LED_BLINK_FAST_INTERVAL * 2)) < LED_BLINK_FAST_INTERVAL
                GPIO.output(led, GPIO.HIGH if on else GPIO.LOW)
                GPIO.output(buz, GPIO.LOW)
            else:
                # Phase 2: LED + Buzzer cung chu ky 1s
                on = (tick % (LED_BLINK_SLOW_INTERVAL * 2)) < LED_BLINK_SLOW_INTERVAL
                GPIO.output(led, GPIO.HIGH if on else GPIO.LOW)
                GPIO.output(buz, GPIO.HIGH if on else GPIO.LOW)
            self._stop_evt.wait(timeout=0.05)
...
def main():
    ...
    hw_alert = HardwareAlertController()
    hw_alert.start()
    ...
    with mp_face_mesh.FaceMesh(
       # tatic_image_mode=False,
        ...
        while True:
            ...
            # ===== CAP NHAT PHAN CUNG LED / BUZZER =====
            hw_alert.set_alert(bool(alert_messages))
