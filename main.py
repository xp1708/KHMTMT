...
import sys
import socket
import requests  # Thêm thư viện HTTP

# ==============================================================================
# 1. CẤU HÌNH BLYNK HTTP
# ==============================================================================
BLYNK_AUTH_TOKEN = "0cS7u48kkbPHi_iecesgzfkIrjOFAUf5"
BLYNK_SERVER     = "https://sgp1.blynk.cloud"  # Server SGP1 theo Blynk Console của bạn


# ==============================================================================
# 1B. DRIVER GPIO CẢNH BÁO PHẦN CỨNG (Gộp từ main_gpio_alert.py)
# ==============================================================================
# GPIO17 (pin 11): LED vàng
# GPIO27 (pin 13): Active buzzer (HIGH = kêu)
# Pattern:
#   - 0..2s: LED nhấp nháy 0.25s ON / 0.25s OFF, buzzer OFF
#   - Sau 2s: LED + buzzer đồng bộ 0.5s ON / 0.5s OFF
#   - Hết cảnh báo: tắt ngay và reset thời gian
_gpio_yellow = None
_gpio_buzz = None
_gpio_ok = False
_gpio_alert_started_at = None

_GPIO_PHASE1_DURATION = 2.0
_GPIO_PHASE1_HALF = 0.25
_GPIO_PHASE2_HALF = 0.5
...
try:
    ...
    _gpio_yellow = LED(_GPIO_YELLOW_PIN, initial_value=False)
    _gpio_buzz = Buzzer(_GPIO_BUZZER_PIN, initial_value=False)
    _gpio_ok = True
    print(f"[GPIO_ALERT] OK — LED vang GPIO{_GPIO_YELLOW_PIN}, Buzzer GPIO{_GPIO_BUZZER_PIN}", flush=True)
except Exception as e:
    print(
        f"[GPIO_ALERT] Khong khoi tao duoc GPIO ({type(e).__name__}: {e}). "
        f"Bo qua canh bao phan cung, main.py van chay binh thuong.",
        flush=True,
    )


def gpio_alert_update(alert_active: bool) -> None:
    """Cập nhật LED vàng + buzzer. Gọi mỗi frame trong vòng lặp chính."""
    global _gpio_alert_started_at

    if not _gpio_ok:
        return

    if not alert_active:
        if _gpio_alert_started_at is not None:
            _gpio_alert_started_at = None
            _gpio_yellow.off()
            _gpio_buzz.off()
        return
    ...
    if _gpio_alert_started_at is None:
        _gpio_alert_started_at = now_mono

    elapsed = now_mono - _gpio_alert_started_at

    if elapsed < _GPIO_PHASE1_DURATION:
        on = (int(elapsed / _GPIO_PHASE1_HALF) % 2) == 0
        _gpio_yellow.on() if on else _gpio_yellow.off()
        _gpio_buzz.off()
    else:
        e2 = elapsed - _GPIO_PHASE1_DURATION
        on = (int(e2 / _GPIO_PHASE2_HALF) % 2) == 0
        if on:
            _gpio_yellow.on()
            _gpio_buzz.on()
        else:
...
def gpio_alert_cleanup() -> None:
    """Tắt LED + buzzer và giải phóng GPIO khi thoát chương trình."""
    if not _gpio_ok:
        return
    try:
        _gpio_yellow.off()
...
# ==============================================================================
# 2. NGƯỠNG HÀNH VI GIÁM SÁT (Giữ nguyên)
# ==============================================================================
CAMERA_INDEX = 0
...
# ==============================================================================
# 4. HÀM BLYNK HTTP (Thay thế toàn bộ BlynkLib)
# ==============================================================================
def blynk_write(pin, value):
    """
    Ghi một giá trị lên Virtual Pin qua Blynk HTTP API.
    Ví dụ: blynk_write(1, 5) → ghi số 5 lên V1
    """
    url = f"{BLYNK_SERVER}/external/api/update?token={BLYNK_AUTH_TOKEN}&v{pin}={value}"
    try:
        resp = requests.get(url, timeout=3)
        if resp.status_code == 200:
            print(f"[BLYNK HTTP] V{pin} = {value} ✓")
        else:
            print(f"[BLYNK HTTP] V{pin} lỗi: {resp.status_code} - {resp.text}")
    except requests.exceptions.ConnectionError:
        print(f"[BLYNK HTTP] Không có internet khi ghi V{pin}")
    except requests.exceptions.Timeout:
        print(f"[BLYNK HTTP] Timeout khi ghi V{pin}")
    except Exception as e:
        print(f"[BLYNK HTTP] Lỗi không xác định V{pin}: {e}")


def blynk_write_batch(pin_value_dict):
    """
    Ghi nhiều Virtual Pin cùng lúc (dùng khi sync định kỳ).
    Ví dụ: blynk_write_batch({1: 3, 2: 1, 3: 0, 4: 2, 5: 0})
    """
    params = "&".join([f"v{pin}={val}" for pin, val in pin_value_dict.items()])
    url = f"{BLYNK_SERVER}/external/api/batch/update?token={BLYNK_AUTH_TOKEN}&{params}"
    try:
        resp = requests.get(url, timeout=5)
        if resp.status_code == 200:
            print(f"[BLYNK HTTP] Batch ghi thành công: {pin_value_dict}")
        else:
            print(f"[BLYNK HTTP] Batch lỗi: {resp.status_code} - {resp.text}")
    except Exception as e:
        print(f"[BLYNK HTTP] Lỗi batch: {e}")


def check_blynk_connection():
    """Kiểm tra kết nối tới Blynk server bằng cách đọc V0."""
    url = f"{BLYNK_SERVER}/external/api/get?token={BLYNK_AUTH_TOKEN}&v0"
    try:
        resp = requests.get(url, timeout=5)
        if resp.status_code == 200:
            print("[BLYNK HTTP] Kết nối Blynk thành công!")
            return True
        else:
            print(f"[BLYNK HTTP] Lỗi kết nối: {resp.status_code} - {resp.text}")
            return False
    except Exception as e:
        print(f"[BLYNK HTTP] Không thể kết nối Blynk: {e}")
        return False


# ==============================================================================
# 5. HÀM BỔ TRỢ HỆ THỐNG (Giữ nguyên)
# ==============================================================================
def signal_handler(sig, frame):
    global system_state
    print(f"\n[SIGNAL] Nhận tín hiệu {sig}. Chuyển sang SAVING_DATA...")
...
# ==============================================================================
# 6. HÀM GỬI DỮ LIỆU BLYNK (Dùng HTTP thay BlynkLib)
# ==============================================================================
def send_data_to_blynk_final(current_status):
    """Gửi toàn bộ dữ liệu cuối cùng lên Blynk trước khi thoát."""
    global count1, count2, count3, count4, count5

    print("\n" + "=" * 60)
    print("[BLYNK HTTP] ĐANG GỬI DỮ LIỆU CUỐI CÙNG TRƯỚC KHI THOÁT...")
    print(f"  - V0 (Trạng thái): {current_status}")
    print(f"  - V1 (Buồn ngủ):   {count1}")
    print(f"  - V2 (Nhìn lệch):  {count2}")
    print(f"  - V3 (Vắng mặt):   {count3}")
    print(f"  - V4 (Điện thoại): {count4}")
    print(f"  - V5 (Bất thường): {count5}")
    print("=" * 60)

    if not blynk_enabled:
        print("[BLYNK HTTP] Blynk không được bật.")
        return

    # Ghi trạng thái V0
    blynk_write(0, current_status)

    # Ghi toàn bộ bộ đếm V1-V5 cùng lúc
    blynk_write_batch({1: count1, 2: count2, 3: count3, 4: count4, 5: count5})
    print("[BLYNK HTTP] Gửi dữ liệu cuối cùng hoàn tất!")


def increment_counter(pin):
    """Tăng bộ đếm và gửi thời gian thực lên Blynk qua HTTP."""
    global count1, count2, count3, count4, count5
    val = 0
    if pin == 1:
        count1 += 1; val = count1
...
# ==============================================================================
# 7. TIẾN TRÌNH CHÍNH (MAIN FUNCTION)
# ==============================================================================
def main():
    global system_state, cap, blynk_enabled, status
    global count1, count2, count3, count4, count5
    ...
    if not check_internet():
        print("[ERROR] Không có internet. Tiếp tục chạy không có Blynk.")
        blynk_enabled = False
    else:
        # Kiểm tra kết nối Blynk HTTP
        blynk_enabled = check_blynk_connection()
    ...
    if ok_startup:
        is_low_startup, mean_s, std_s = check_lighting(startup_frame, LOW_LIGHT_BRIGHTNESS_THRESHOLD, LOW_LIGHT_NOISE_THRESHOLD)
        if is_low_startup:
            print(f"[CANH BAO] Ánh sáng yếu. mean={mean_s:.0f}, std={std_s:.0f}")
        ...
        while system_state == "RUNNING":
            ...
            if not ok or frame is None:
                ...
                if camera_fail_count >= CAMERA_RETRY_LIMIT:
                    print("[ERROR] Mất camera. Đang thử lại...")
                    cap.release()
                    time.sleep(2.0)
                    cap = cv2.VideoCapture(CAMERA_INDEX, cv2.CAP_V4L2)
                    camera_fail_count = 0
                    if not cap.isOpened():
                        print("[ERROR] Không thể kết nối lại camera. Thoát.")
                        break
                continue
            ...
            if results.multi_face_landmarks:
                ...
                if partial_face_since is not None and (now - partial_face_since) >= PARTIAL_FACE_SECONDS:
                    ...
                    if not state_partial_face_alert:
                        increment_counter(5)  # ← Bỏ tham số blynk/blynk_enabled
            ...
            # ===== GỬI V0 (TRẠNG THÁI) KHI THAY ĐỔI =====
            if blynk_enabled and status != last_blynk_status:
                blynk_write(0, status)
                last_blynk_status = status

            # ===== ĐỒNG BỘ ĐỊNH KỲ V1-V5 MỖI 60 GIÂY =====
            if blynk_enabled and (now - last_blynk_send_time) >= 60.0:
                print("[BLYNK HTTP] Đồng bộ định kỳ V1-V5...")
                blynk_write_batch({1: count1, 2: count2, 3: count3, 4: count4, 5: count5})
                last_blynk_send_time = now

            # Cập nhật LED vàng + buzzer phần cứng theo trạng thái cảnh báo hiện tại
            # Dùng cùng điều kiện với cảnh báo âm thanh phần mềm để tránh xung đột logic.
            hardware_alert_active = bool(alert_messages)
            gpio_alert_update(hardware_alert_active)
            ...
            if key == ord("q"):
                print("[INFO] Người dùng nhấn Q. Đang dừng...")
                system_state = "SAVING_DATA"
                break
...
if __name__ == "__main__":
    try:
        ...
        print("\n[INFO] Nhận tín hiệu ngắt.")
    finally:
        print("\n[INFO] Giải phóng camera và cửa sổ...")
        if cap is not None:
            try:
                cap.release()
            except Exception as e:
                print(f"[ERROR] Lỗi giải phóng camera: {e}")
        cv2.destroyAllWindows()

        # Dọn dẹp GPIO phần cứng cảnh báo
        gpio_alert_cleanup()

        # Gửi dữ liệu cuối cùng lên Blynk
        if blynk_enabled:
            send_data_to_blynk_final(status)
        else:
            print("[BLYNK] Không gửi vì chưa kết nối.")

        print("[INFO] Đã thoát hệ thống hoàn toàn.")
